...
import { useSocketStore } from "../store/socketStore"; // <-- NEW IMPORT

export const useInventorySocket = () => {
  const queryClient = useQueryClient();
  const token = useAuthStore((state) => state.token);
  const setIsConnected = useSocketStore((state) => state.setIsConnected); // <-- NEW STATE SETTER
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      setIsConnected(true); // <-- UPDATE STORE TO TRUE
    });
    ...
    socket.on("disconnect", (reason) => {
      ...
      setIsConnected(false); // <-- UPDATE STORE TO FALSE
    });
    ...
    return () => {
      socket.disconnect();
      setIsConnected(false); // Ensure it's false on cleanup
    };
  }, [queryClient, token, setIsConnected]);
};
